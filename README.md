...
| `--upgrade` | Update kit-managed files. Ones you edited are kept and reported (the kit's copy lands next to them as `<file>.kit-new`, never over an earlier one — then `.kit-new.1`, …); so is a file at a kit path the kit never installed; project files are never touched. Needs `sha256sum`, `shasum` or `python3` |
...
The `.kit-manifest` file tracks which files are kit-managed, so upgrades know what to update and what to skip. `.kit-baseline` records the hash of what the kit last installed at each path: `--upgrade` updates a file you haven't touched, keeps one you edited, and when the kit also changed it writes the new version as `<file>.kit-new` for you to merge. Installs from before `.kit-baseline` existed can't tell an edit from an older kit file, so their first upgrade replaces changed files and keeps the previous copies in `.kit-backup/<timestamp>/`. Once `.kit-baseline` exists, a file at a kit path it doesn't list — say your own `scripts/validate.sh` — was never the kit's: it's kept and the kit's version offered as `<file>.kit-new`. `CLAUDE.md` stays on the template it came from (the one `.kit-baseline` records, else the one its first line names); when neither tells, the upgrade leaves it alone and says so — pass `--template` for a stack template, or merge the kit's copy by hand. Only files `.kit-baseline` shows the kit wrote are ever reported as stale.
